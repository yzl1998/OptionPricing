...
def whether_exercise(S, K, k, coefficients, test_functions):
    ...
    return max(S - K, 0.0) > continue_val
...
def payoff(exercise_val, path, i, K, m):
    for j in range(0, m):
        exercise_val[j] = max(0.0, path[j][i] - K)
        ...
        for j in range(0, self.m):
            ...
            for i in range(0, self.n):
                ...
                if whether_exercise(self.S, self.K, i, coefficients, test_functions):
                    sum += max(self.S - self.K, 0.0)*math.exp(- self.r*t)
...
def main(m, n):
    ...
    for i in range(0, m):
        for j in range(0, n):
            ao.dW[i][j] = np.random.normal(0.0, 1.0)
    """
    ...
    print("S V_lower = {}".format(ao.S))
    print("delta dV/dS0 = {}".format(ao.delta))
    print("gamma dV2/dS02 = {}".format(ao.gamma))
    print("vegaBS dV/dsigma = {}".format(ao.vegaBS))
    print("vannaBS dV2/dsigma dS0 = {}".format(ao.vannaBS))

    for i in range(0, n):
        ao.S = ao.S0
        """
        vega[i] = 1
        print("dV2/dsigma[{}] dS0 = {}".format(i, vegaBS))
        vegaBS += St
        vega[i] = 0
        """
    # print(coefficients)
